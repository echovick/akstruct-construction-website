...
new #[Layout('layout.web')] class extends Component {
    ...
    {
        return [
            'relatedProjects' => Project::where('category_id', $this->project->category_id)
                    ...
                    <div class="bg-gray-50 rounded-lg p-8 sticky top-8">
                        ...
                        <dl class="space-y-4">
                            ...
                            @if($project->contractor)
                                <div class="border-b border-gray-200 pb-4">
                                    ...
                                    <dd class="mt-1 text-lg font-semibold text-gray-900">{{ $project->contractor }}</dd>
                                </div>
                            @endif
                            ...
                            <div class="border-b border-gray-200 pb-4">
                                ...
                                <dd class="mt-1 text-lg font-semibold text-gray-900">{{ $project->category->name ?? 'N/A' }}</dd>
                            </div>
                            ...
                            @if($project->completed_at)
                                <div class="border-b border-gray-200 pb-4">
                                    ...
                                    <dd class="mt-1 text-lg font-semibold text-gray-900">{{ $project->completed_at->format('F Y') }}</dd>
                                </div>
                            @endif
                            ...
                            @if($project->cost)
                                <div>
                                    ...
                                    <dd class="mt-1 text-lg font-semibold text-gray-900">₦{{ number_format($project->cost, 2) }}</dd>
                                </div>
                            @endif
                        </dl>
                        ...
                        <div class="mt-8 space-y-3">
                            @if($project->case_study_pdf)
                                <a href="{{ asset($project->case_study_pdf) }}" download class="w-full bg-orange-500 text-white px-6 py-3 rounded-lg font-semibold hover:bg-orange-600 transition text-center block">
                                    Download Case Study
                                </a>
                            @endif

                            @if($project->completion_certificate)
                                <a href="{{ asset($project->completion_certificate) }}" download class="w-full border-2 border-gray-300 text-gray-700 px-6 py-3 rounded-lg font-semibold hover:border-gray-400 transition text-center block">
                                    View Certificate
                                </a>
                            @endif

                            <a href="{{ route('contact') }}" class="w-full border-2 border-orange-500 text-orange-500 px-6 py-3 rounded-lg font-semibold hover:bg-orange-500 hover:text-white transition text-center block">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($project->gallery_images as $index => $image)
                        <div @click="openLightbox({{ $index }})" class="relative group cursor-pointer overflow-hidden rounded-lg bg-gray-200">
                            <img src="{{ asset($image) }}"
                                 alt="{{ $project->title }} - Image {{ $index + 1 }}"
                                 class="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500"
                                 onerror="this.src='https://via.placeholder.com/800x600/1a365d/ffffff?text=Image+Unavailable'">
        ...
        <script>
            function galleryLightbox() {
                return {
                    ...
                    images: [
                        @foreach($project->gallery_images as $index => $image)
                            { src: '{{ asset($image) }}', alt: '{{ $project->title }} - Image {{ $index + 1 }}' }{{ $loop->last ? '' : ',' }}
                        @endforeach
                    ],
                    get currentImage() {
                    ...
                    <div class="space-y-8">
                        @foreach($project->timeline as $milestone)
                            <div class="flex items-center">
                                <div class="w-16 h-16 bg-orange-500 rounded-full flex items-center justify-center text-white font-bold text-sm">
                                    {{ $milestone['phase'] ?? '' }}
                                </div>
                    ...
                    @foreach($relatedProjects as $relatedProject)
                        <div class="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition">
                            @if($relatedProject->featured_image || $relatedProject->image_path)
                                <img src="{{ asset($relatedProject->featured_image ?? $relatedProject->image_path) }}"
                            ...
                            @endif
                            <div class="p-6">
                                <span class="text-orange-500 text-sm font-semibold">{{ $relatedProject->category->name ?? 'Project' }}</span>
                                <h3 class="text-xl font-bold text-gray-900 mt-2">{{ $relatedProject->title }}</h3>
                                <p class="text-gray-600 mt-2">{{ Str::limit($relatedProject->short_description ?? $relatedProject->description, 80) }}</p>
                                <a href="{{ route('projects.show', $relatedProject) }}" class="inline-flex items-center text-orange-500 font-semibold mt-4 hover:text-orange-600">
                                    View Project
