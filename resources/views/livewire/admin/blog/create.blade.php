...
new #[Layout('layout.admin')] class extends Component {
    ...
    public function save()
    {
        ...
        $post->published_at = $this->published ? now() : null;
        $post->author_id = auth()->id();

        if ($this->featured_image) {
            $post->featured_image = $this->featured_image->store('blog', 'public');
        }
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex justify-between items-center">
        <h2 class="text-2xl font-semibold text-gray-800">Create New Blog Post</h2>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                ...
                <div class="text-sm text-gray-500">
                    <p>Author: {{ auth()->user()->name ?? 'Unknown' }}</p>
                    <p>Created: {{ now()->format('M d, Y H:i') }}</p>
                </div>
            </div>
        </div>
    </form>
